...
names(zmerg)     <- c("factorLevelId", "Global.value", "Local.value", "Multiplier")
...
smbs1        <- fromJSON("/repos/RCB4Cloud/Reference/Data/prod_out/p360/SIMPLEMEANSBYSUBSITE12-18382-SC-outputToP360.json")
...
zmergbs      <- merge(smbs1gfactor,smbs1ffactor, by='id', all=T)

zmergbs$Multiplier <- paste0(round(zmergbs$value.y/zmergbs$value.x))
names(zmergbs)     <- c("factorLevelId", "Global.value", "Local.value", "Multiplier")

zmergbs$Multiplier[which(zmergbs$Multiplier == "1e+10")] <- paste(c(1,rep(0,10)), sep="",collapse="")
zmergbs$Multiplier[which(zmergbs$Multiplier == "1e+09")] <- paste(c(1,rep(0, 9)), sep="",collapse="")
zmergbs$Multiplier[which(zmergbs$Multiplier == "1e+08")] <- paste(c(1,rep(0, 8)), sep="",collapse="")
zmergbs$Multiplier[which(zmergbs$Multiplier == "1e+06")] <- paste(c(1,rep(0, 6)), sep="",collapse="")
zmergbs$Multiplier[which(zmergbs$Multiplier == "1e+05")] <- paste(c(1,rep(0, 5)), sep="",collapse="")
